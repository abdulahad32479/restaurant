...
files.forEach(file => {
  ...
  // Top level structure wrapping `animate-fade-in pb-10`
  if (!content.includes('bg-[#f4f6f8]')) {
    content = content.replace(/className="space-y-6 animate-fade-in pb-10"/g, 'className="animate-fade-in -m-6 p-6 min-h-screen bg-[#f4f6f8] font-sans text-slate-800 space-y-6"');
  }
